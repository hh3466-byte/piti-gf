...
export const SupermarketSelfScanView: React.FC<SupermarketSelfScanViewProps> = ({
  ...
  const handleFridgeShelfPhotoCapture = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      onAnalyze({
        imageBase64: optimizedBase64,
        textPrompt: 'צלמתי את המקרר / המדף שלי. זהה את כל המצרכים הבטוחים לצליאק וללא גלוטן שנראים בתמונה, והצע לי 2-3 רעיונות קלים ומהירים ב-3 דקות של שף דלה פופו להכנה ממה שיש!',
        mimeType: 'image/jpeg',
      });
    } catch (err) {
      const reader = new FileReader();
