...
- "RED" (אור אדום - אסור בתכלית): המאכל עשיר ברכיבים מתסיסים (פרוקטנים כמו שום/בצל/חיטה, גלקטנים/קטניות, לקטוז מחלב ניגר, מניטול מפטריות/כרובית, סורביטול, עודף פרוקטוז גבוה כמו תפוח/אגס/דבש, או ממתיקים כוהליים כמו קסיליטול). אסור לחלוטין לפיתי!
...
1. קבע תמיד status: "GREEN" (או YELLOW אם מדובר בפחמימה/בצק הדורש שלב 2) — ספק תמיד מענה קולינרי מעודד, חיובי, מפורט ובטוח לפיתי!
...
`;
            ...
            config: {
              ...
              responseSchema: {
                ...
                properties: {
                  ...
                  shortVerdict: {
                    type: Type.STRING,
                    description: 'Short impactful 1-sentence verdict in Hebrew (e.g. אור ירוק! מותר ובטוח לפיתי)',
                  },
                  detailedExplanation: {
                    type: Type.STRING,
  ...
  app.post('/api/fridge-chef', async (req, res) => {
    try {
      ...
      const systemInstruction = `
אתה שף חירום קליני מומחה ל-SIBO (צמיחת יתר של חיידקים במעי הדק) עבור פיתי.
            ...
            config: {
              ...
              responseSchema: {
                ...
                properties: {
                  ...
                  calmMessage: { type: Type.STRING, description: 'משפט מרגיע ומחבק לפיתי שיוריד לחץ' },
...
אתה עונה לאבא של ניר או לפיתי עצמה, בעברית חמה, מקצועית, ברורה ומעשית.
